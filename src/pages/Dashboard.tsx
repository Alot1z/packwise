...
import {
  Luggage, Search, Plus, Trash2, Copy, Star, Shirt, Layers, Sparkles, Share2, Download, Upload, LogOut, CalendarDays, MapPin, SlidersHorizontal, Check, X, Archive
} from "lucide-react";
...
const STARTER_TEMPLATES = [
  { name: "Weekend City — 2 nights", tag: "weekend", desc: "Light city break", items: [
    ...
    { name:"Charger + cable", category:"Tech", quantity:1, essential:true },
    { name:"Passport / ID", category:"Documents", quantity:1, essential:true },
  ]},
  { name: "Business 3-day", tag: "work", desc: "Meetings & travel", items: [
    ...
    { name:"Laptop + charger", category:"Tech", quantity:1, essential:true },
    { name:"Notebook", category:"Documents", quantity:1, essential:false },
  ]},
  { name: "Beach Week", tag: "summer", desc: "Sun, swim, slow days", items: [
...
export default function Dashboard() {
  ...
  const handleCreateTrip = async()=>{
    if(!newTrip.title.trim() || !newTrip.destination.trim()) return toast.error("Title and destination required");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-20 backdrop-blur-xl bg-background/80 border-b border-border">
        <div className="max-w-[1360px] mx-auto px-4 sm:px-6 h-[56px] flex items-center gap-3">
          <div className="flex items-center gap-2">
            <div className="size-8 rounded-lg bg-primary text-primary-foreground grid place-items-center"><Luggage className="size-4" /></div>
            <span className="text-[15px] font-semibold tracking-tight" style={serif}>FULLPACK</span>
            <span className="hidden md:inline text-[11px] font-mono uppercase tracking-[0.12em] text-muted-foreground ml-1">Athena · Open</span>
          </div>
          <div className="hidden sm:flex items-center gap-2 ml-4">
            ...
            <Badge variant="outline" className="rounded-full font-normal">{progress}% packed</Badge>
          </div>
      ...
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 py-6 grid lg:grid-cols-[300px_1fr] gap-6 items-start">
        {/* Sidebar */}
        <aside className="lg:sticky lg:top-[68px] space-y-4">
          <Card className="rounded-[20px] p-4 border-border bg-card">
            <div className="flex items-center justify-between">
              <div className="font-semibold" style={serif}>Trips</div>
              <Dialog open={newTripOpen} onOpenChange={setNewTripOpen}>
                ...
                <DialogContent className="rounded-2xl">
                  ...
                  <div className="grid gap-3">
                    <Input placeholder="Title — e.g. Kyoto spring" value={newTrip.title} onChange={e=>setNewTrip({...newTrip, title:e.target.value})} />
                    ...
                    <Input placeholder="Purpose — work / leisure / family" value={newTrip.purpose} onChange={e=>setNewTrip({...newTrip, purpose:e.target.value})} />
            ...
            <div className="mt-3 space-y-2 max-h-[42vh] overflow-auto pr-1">
              {filteredTrips.length===0 && <div className="text-sm text-muted-foreground py-6 text-center border border-dashed rounded-xl">No trips yet</div>}
              {filteredTrips.map(t=>{
          ...
          <Card className="rounded-[20px] p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">Save or apply reusable packs.</p>
            <div className="mt-3 space-y-2">
              {templates.map(t=>(
                <div key={String(t._id)} className="flex items-center gap-2 rounded-xl border p-2.5 bg-white">
                  ...
                  <Button variant="ghost" size="icon" className="size-7" onClick={async()=>{ await deleteTemplate({ templateId: t._id as any }); toast.success("Deleted"); }}><Trash2 className="size-3.5" /></Button>
                </div>
              ))}
              {templates.length===0 && <div className="text-xs text-muted-foreground">No templates yet — save one or seed starters.</div>}
        ...
        <main className="min-w-0 space-y-4">
          {!activeTrip ? (
            <Card className="rounded-[24px] p-10 text-center border-dashed">
              <div className="size-12 mx-auto rounded-2xl bg-secondary grid place-items-center"><Luggage className="size-6" /></div>
              <h2 className="text-[28px] mt-4" style={serif}>Begin your first <span className="italic font-light">pack</span></h2>
              <p className="text-sm text-muted-foreground mt-2 max-w-[48ch] mx-auto">Create a trip to unlock checklists, outfits and progress. Everything is local-first and exportable.</p>
            ...
            <>
              ...
              <div className="athena-paper rounded-[24px] border border-white/60 p-5 sm:p-6 shadow-[0_12px_40px_-20px_oklch(0.3_0.05_42/0.25)]">
                <div className="flex flex-wrap items-start gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="hidden sm:block">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">{items.filter(i=>i.packed).length} / {items.length} items</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  ...
                  <Button variant="outline" size="sm" className="rounded-full bg-white" onClick={async()=>{ const id=await duplicateTrip({ tripId: activeTrip._id as any }); setActiveTripId(String(id)); toast.success("Duplicated"); }}><Copy className="size-3.5" /> Duplicate</Button>
                  ...
                  }}><Share2 className="size-3.5" /> Copy JSON</Button>
              ...
              <Tabs defaultValue="items" className="w-full">
                <TabsList className="rounded-full bg-secondary p-1 h-auto">
                  <TabsTrigger value="items" className="rounded-full gap-1.5 data-[state=active]:bg-white"><Layers className="size-3.5" /> Items</TabsTrigger>
                  <TabsTrigger value="outfits" className="rounded-full gap-1.5 data-[state=active]:bg-white"><Shirt className="size-3.5" /> Outfits</TabsTrigger>
                  <TabsTrigger value="share" className="rounded-full gap-1.5 data-[state=active]:bg-white"><Share2 className="size-3.5" /> Share & Export</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="items" className="mt-4 space-y-4">
                  ...
                  <Card className="rounded-2xl p-3">
                    <div className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-2">Add item</div>
                    <div className="grid sm:grid-cols-[1fr_140px_90px_auto] gap-2">
                      <Input placeholder="Item name — e.g. Merino tee" value={addRow.name} onChange={e=>setAddRow({...addRow, name:e.target.value})} />
                    ...
                    <div className="flex gap-2 mt-2">
                      <Input placeholder="Notes (optional)" value={addRow.notes} onChange={e=>setAddRow({...addRow, notes:e.target.value})} className="h-8 text-sm" />
                  ...
                  {Object.keys(grouped).length===0 ? (
                    <Card className="rounded-2xl p-8 text-center border-dashed">
                      <div className="text-sm text-muted-foreground">No items match your filters — or nothing packed yet.</div>
                    </Card>
                    ...
                    <Card key={cat} className="rounded-2xl overflow-hidden">
                      ...
                      <div className="divide-y">
                        {list.map(item=>(
                          <div key={String(item._id)} className={`flex items-center gap-3 px-3 sm:px-4 py-3 ${item.packed ? "bg-emerald-50/50" : "bg-white"}`}>
                            ...
                            <Button variant="ghost" size="icon" className="size-7 shrink-0" onClick={async()=>{ const n=prompt("Rename item", item.name); if(n && n.trim()) await updateItem({ itemId: item._id as any, name: n }); }}><SlidersHorizontal className="size-3.5" /></Button>
                ...
                <TabsContent value="outfits" className="mt-4 space-y-4">
                  <Card className="rounded-2xl p-4">
                    <div className="font-semibold" style={serif}>Compose outfit</div>
                    <p className="text-xs text-muted-foreground">Pick items from this trip — mix and name a day.</p>
                    <div className="grid sm:grid-cols-3 gap-2 mt-3">
                      <Input placeholder="Outfit name — e.g. Arrival" value={outfitName} onChange={e=>setOutfitName(e.target.value)} />
                      <Input placeholder="Day — Mon / Day 2" value={outfitDay} onChange={e=>setOutfitDay(e.target.value)} />
                    ...
                    {selectedIds.length>0 && <div className="text-xs text-muted-foreground mt-2">{selectedIds.length} selected</div>}
                  </Card>
                    ...
                    {outfits.map(o=>(
                      <Card key={String(o._id)} className="rounded-2xl p-4">
                        <div className="flex items-start gap-2">
                          ...
                          <Button variant="ghost" size="icon" className="size-7" onClick={async()=>{ await deleteOutfit({ outfitId: o._id as any }); toast.success("Removed"); }}><Trash2 className="size-3.5" /></Button>
                        </div>
                        ...
                        {o.note && <div className="text-xs text-muted-foreground mt-2">{o.note}</div>}
                      </Card>
                    ))}
                    {outfits.length===0 && <Card className="rounded-2xl p-8 text-center border-dashed text-sm text-muted-foreground sm:col-span-2">No outfits yet — compose from your checklist above.</Card>}
                  </div>
                </TabsContent>

                <TabsContent value="share" className="mt-4 space-y-4">
                  <Card className="rounded-2xl p-5">
                    <div className="font-semibold flex items-center gap-2" style={serif}><Share2 className="size-4" /> Share & portability</div>
                    <div className="grid sm:grid-cols-3 gap-3 mt-4">
                      ...
                      }}><Copy className="size-4" /> Copy JSON</Button>
                      ...
                      }}><Download className="size-4" /> Download JSON</Button>
                    ...
                    <div className="text-xs leading-5 text-muted-foreground">FullPack is open and self-hostable — your data is a plain JSON document you can move anywhere. No proprietary lock-in.</div>
                    <Textarea readOnly value={JSON.stringify({ trip: activeTrip, items: items.slice(0,3), outfits: outfits.slice(0,1), note:"preview — copy full JSON above" }, null, 2)} className="mt-3 font-mono text-xs h-[160px] bg-secondary/40" />
                  </Card>
                </TabsContent>
              </Tabs>
            </>
