import { BookOpen, Cpu, Database, Shield, ExternalLink, HardDrive, FileCode2, GitBranch } from "lucide-react";
...
const pillars = [
  { icon: Shield, title: "Privacy model", desc: "No accounts, no tracking, no telemetry. All data stays in SwiftData on your device. Vision analysis never leaves the phone." },
  { icon: HardDrive, title: "Offline-first", desc: "The app is fully usable without a network. Local storage, local notifications, local recommendations." },
  { icon: Cpu, title: "On-device intelligence", desc: "Rule-based suggestions and Apple Vision classification run locally. No external AI APIs, no paid services." },
  { icon: Database, title: "Backup-safe data", desc: "SwiftData models with automatic migration. Export any trip as JSON to move or share it." },
];
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="max-w-[1180px] mx-auto px-6 pb-10">
        <div className="grid md:grid-cols-2 gap-4">
          {pillars.map((p) => (
            <div key={p.title} className="rounded-2xl border border-border bg-card p-5">
              <div className="size-9 rounded-xl bg-secondary grid place-items-center border border-border/50"><p.icon className="size-[18px]" /></div>
              <div className="font-semibold mt-3.5 text-[15px]" style={serif}>{p.title}</div>
        ...
        <div className="mt-10">
          ...
          <h2 className="mt-2 text-[28px] sm:text-[32px] leading-none tracking-[-0.02em]" style={serif}>Wiki pages</h2>
        </div>
        <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {wikiPages.map((w) => (
            <a key={w.title} href={w.href} target="_blank" rel="noreferrer" className="group rounded-2xl border border-border bg-card p-5 hover:shadow-[0_8px_30px_-12px_oklch(0.3_0.05_42/0.2)] hover:-translate-y-0.5 transition-all">
              <div className="flex items-center justify-between">
                <BookOpen className="size-4 text-muted-foreground" />
                <ExternalLink className="size-3.5 text-muted-foreground opacity-0 group-hover:opacity-100 transition" />
              </div>
              <div className="font-semibold mt-3 text-[15px]" style={serif}>{w.title}</div>
        ...
        <div className="mt-8 rounded-2xl border border-border bg-card p-5 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-muted-foreground">
          <a href={LIVE_REPO} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-foreground"><GitBranch className="size-4" /> Source: Alot1z/packwise</a>
          <a href={LIVE_RELEASES} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-foreground"><FileCode2 className="size-4" /> Releases & artifacts</a>
          <a href={`${WIKI_URL}/Architecture`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-foreground"><Cpu className="size-4" /> Architecture deep dive</a>
