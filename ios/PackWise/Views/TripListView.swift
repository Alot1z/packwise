...
struct TripListView: View {
    ...
    var filtered: [Trip] {
        ...
        return trips.filter { $0.title.lowercased().contains(q) || $0.destination.lowercased().contains(q) }
    }
    ...
    var body: some View {
        NavigationStack {
            Group {
                if trips.isEmpty {
                    ContentUnavailableView {
                        ...
                        Text("Create a trip to access your private dashboard, detailed item views, and day-by-day outfit planning. Everything stays on device and works offline.")
                    } actions: {
                        Button("Create your first trip") { showNew = true }
            ...
            .overlay(alignment: .bottom) {
                if !trips.isEmpty {
                    Text("Stored locally on this device · Private and offline-first")
...
private extension Font {
    static func caption2Monospaced() -> Font { .caption2.monospaced() }
}
