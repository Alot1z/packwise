...
| Visual outfit canvas with drag-and-drop | Listing: "Drag and drop items onto a visual canvas. Plan outfits by day or occasion" | drag item cutouts onto a day/occasion canvas | SwiftUI `.draggable` / `.dropDestination` (iOS 16 `dropDestination(for:action:isTargeted:)` — the iOS 18+ `(for:isEnabled:action:)` variant is iOS 26+, rejected by the iOS 18 target) | 18 | Photo-chip palette (tap or drag onto canvas) + canvas (drag to reorder, drop to insert before, x to remove). Composition order preserved via `Outfit.itemIDs`. Pure logic in `OutfitComposer` (unit-tested). | `Views/TripDetailView.swift` (outfits tab), `Models/Models.swift` (`Outfit.itemIDs`), `Services/OutfitComposer.swift`, `PackWiseTests/PackWiseTests.swift` | IMPLEMENTED — build/archive/IPA gate GREEN (commit `5ad6504`) |
| Save favorite outfit combinations | Listing: "save your favorite combinations" | favorite flag + quick recall | SwiftData attribute (default `false` — lightweight migration) | — | `Outfit.isFavorite` + star toggle in outfit list (leading swipe) and detail toolbar; favorites sort first. Persistence unit-tested. | `Models/Models.swift`, `Views/TripDetailView.swift`, `PackWiseTests/PackWiseTests.swift` | IMPLEMENTED — build/archive/IPA gate GREEN (commit `5ad6504`) |

### 2.8 System integrations (modernization layer)

| FullPack capability | Evidence | Atomic behavior | Apple API | Min iOS | PackWise implementation | Files | Status |
|---|---|---|---|---|---|---|---|
| Siri/Shortcuts actions | Modern native norm | add item, mark packed, create trip | `AppIntents.AppIntent`, `AppEntity` | 16 | **IMPLEMENTED** — `AddInventoryItemIntent`, `MarkPackedIntent`, `CreateTripIntent` + `PackWiseShortcuts` provider; App Group container ensures intents see the same SwiftData store | `AppIntents/PackWiseIntents.swift` | TESTED — 3 behavior tests against in-memory SwiftData (add item, create trip, mark packed) |
...
| Live Activities (departure) | Modern native norm | trip countdown | `ActivityKit` | 16.1 | **IMPLEMENTED** — `TripActivityAttributes` shared app/widget Codable shape; `LiveActivityService` (@MainActor sync/update/end, self-healing `syncAll` on the trips list, hooked after every item mutation and on trip delete); `TripLiveActivityWidget` (lock screen + Dynamic Island, warm design tokens inlined for the widget target); `NSSupportsLiveActivities` in Info.plist. 24h lead window, ongoing trips stay live, auto-end after trip end. Pure policy in `TripActivitySupport` (6 unit tests); `Activity` bridged `@retroactive @unchecked Sendable` for Swift 6 strict. | `Models/TripActivity.swift`, `Services/LiveActivityService.swift`, `Widgets/TripLiveActivityWidget.swift`, `Views/TripDetailView.swift`, `Views/TripListView.swift`, `Info.plist`, `PackWiseTests/PackWiseTests.swift` | IMPLEMENTED — build/archive/IPA gate GREEN (commit `a7f32d1`) |
...
| App Intents / Widgets / Live Activities / Liquid Glass | IMPLEMENTED (Widgets + App Intents + Live Activities) / DESIGNED (Liquid Glass — iOS 26) |
