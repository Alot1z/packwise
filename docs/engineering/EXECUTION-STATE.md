...
> all 17 recent GitHub Actions runs. Latest iOS run **31320480575** (commit
> `5086af8`) failed at `Generate Xcode project` step 7 in 0s with `xcodegen
> generate failed (exit 1): Parsing project spec failed: Decoding failed at
> "path": Nothing found`. **Read the actual XcodeGen 2.46.0 source to find the
> TRUE root cause**: `Plist.init` does `path = try
> jsonDictionary.json(atKeyPath: "path")` which **throws keyNotFound
> ("Nothing found") when the `path:` key is absent** — and both
> `entitlements:` blocks (app + widget) in `ios/project.yml` declared
> `properties:` without `path:`. That single missing key has been the blocker
> in EVERY iOS run (the `excludes: [Info.plist]` bare-filename and
> `options.entitlements` theories were symptoms, not causes). Fixed by adding
> `path: PackWise/PackWise.entitlements` and
> `path: PackWise/Widgets/PackWiseWidget.entitlements` — XcodeGen's
> `FileWriter.writePlists()` generates the file from `properties` and
> `PBXProjGenerator` wires `CODE_SIGN_ENTITLEMENTS` automatically (verified in
> source). Earlier `excludes` glob correction (bare filename → `"**/Info.plist"`)
> kept + documented. Also installed **actionlint v1.7.7** (real GH Actions
> schema linter) — all 3 workflows exit 0. Wiki sync consistently SUCCESS 6/6;
> the prior `sync-wiki | exit 128` was a transient git-clone rate-limit
> (non-blocking step). Local sweep all green: tsc 0, bun build 3.66s, bash -n
> 3:3, js-yaml 4:4, convex dev --once 2.44s, no bare Info.plist in excludes,
> entitlements now carry required `path:` keys. Linux `xcodegen generate` is
> unreproducible (macOS-only Mach-O; `Exec format error`); the macOS CI runner
> is the authoritative gate. **Pending: next macOS CI run** — with
> entitlements.path fixed, `xcodegen generate` should finally pass, then
> iOS-18 Swift compiles + tests + archive + verify-ipa.)
>
...
| 21 · XcodeGen TRUE root cause + entitlements.path fix (session 15) | **DONE** — comprehensive scan of all 17 recent GitHub Actions runs; latest iOS run **31320480575** (commit `5086af8`) failed at `Generate Xcode project` in 0s with `Decoding failed at "path": Nothing found`. **Read XcodeGen 2.46.0 source** (Plist.swift / FileWriter.swift / PBXProjGenerator.swift) and found the definitive root cause: `Plist.init` throws `keyNotFound` ("Nothing found") when the `path:` key is missing, and BOTH `entitlements:` blocks in `ios/project.yml` (app + widget) declared `properties:` without `path:` — this has been the true blocker in EVERY iOS run (earlier `excludes: [Info.plist]`, `options.entitlements`, and sdk: deps theories were symptoms). Fixed: added `path: PackWise/PackWise.entitlements` (app) and `path: PackWise/Widgets/PackWiseWidget.entitlements` (widget); XcodeGen `FileWriter.writePlists()` generates the file from properties and `PBXProjGenerator` sets `CODE_SIGN_ENTITLEMENTS` automatically (verified in source). Also kept the earlier `excludes` glob correction and installed **actionlint v1.7.7** — all 3 workflows validate against GitHub's real workflow schema (exit 0). Wiki sync consistently SUCCESS 6/6; the prior `sync-wiki | exit 128` was a transient git-clone rate-limit, correctly non-blocking. Local sweep all green: tsc 0, bun build 3.66s, bash-n 3:3, js-yaml 4:4, convex dev --once 2.44s, entitlements now carry the required `path:` keys, no bare Info.plist in excludes. Linux `xcodegen generate` is unreproducible (macOS-only Mach-O); the macOS CI runner is the authoritative gate. **Pending: next macOS CI run validates the fix** — with entitlements.path present, `xcodegen generate` should finally pass, then the iOS-18 Swift sources compile + test + archive + `verify-ipa.sh`. |
